...
let lifeCompleted = false; // whether life (生活知識) round is completed
...
function draw() {
  ...
  text('記分板', 28, 20);
  textSize(18);
  text(score + ' / ' + SCORE_MAX, 28, 40);
  textSize(12);
  // status for roles
  const mathStatus = drowned.micky ? '數學: 被淹沒' : '數學: 可出題';
  const lifeStatus = drowned.right ? '生活: 被淹沒' : '生活: 可出題';
  text(mathStatus, 28, 58);
  text(lifeStatus, 28, 74);
  pop();
...
      // first do math (micky), then english (character 4)
  if (!mathCompleted && !drowned.micky) {
...
          currentQuestionIndex = questions.indexOf(qobj);
      ...
      } else if (!mathCompleted && drowned.micky) {
        mickyDialogText = '被海浪淹沒中…請先答對生活常識題復原';
      } else if (!lifeCompleted && !lifeAsked && (mathCompleted || rightMoved) && !drowned.right) {
      // select a life-knowledge question for the RIGHT character (only after math round or after it has moved)
      const lifeQuestions = questions.filter(q => q.題型 === '生活知識' || q.題型.toLowerCase() === 'english');
...
      }
  } else if (!lifeCompleted && drowned.right) {
      // if drowned, prompt the player to answer a math question to recover
      rightDialogText = '被海浪淹沒中…請先答對數學題復原';
    } else {
...
          showingCompletionMessage = true;
          ...
          const currentAsker = (currentQuestionType === 'life') ? 'right' : (currentQuestionType === 'math') ? 'micky' : null;
          ...
          const asker = (currentQuestionType === 'life') ? 'right' : (currentQuestionType === 'math') ? 'micky' : null;
          if (asker) {
            drowned[asker] = true;
            ...
            leftFarDialogText = questions[currentQuestionIndex].提示 || '提示：再想想';
...
function handleReviveChoice(yes) {
  ...
  if (yes) {
    ...
    if (role && drowned[role]) {
      ...
      waveFloodTarget = 0;
      // celebration
      emitRainbowTickets((role === 'right') ? rightX : (role === 'micky') ? mickyX : leftFarX, groundY - displayH / 2 - 40, 30);
    }
  } else {
